...
$( function() {
    ...
    function buildGame( x ) {
		...
		if ( ez == false ) {
            letters.push('a','e','i','s','h','o','u','z','q','w','k');	
        } else {
            letters.push('i','s','h');	
        }
        ...
        $letters.find( 'li' ).each( function( i ) {
            if (ez == false) {
				var top   = ( $models.position().top ) + ( Math.random() * 100 ) + 100,
					left  = ( $models.offset().left - $container.offset().left - 300) + ( Math.random() * 10 ) + ( i * letterWidth),
					angle = ( Math.random() * 50 ) - 10;		
			} else {
				var top   = ( $models.position().top ) + ( Math.random() * 100 ) + 100,
					left  = ( $models.offset().left - $container.offset().left - 100) + ( Math.random() * 10 ) + ( i * letterWidth),
					angle = ( Math.random() * 50 ) - 10;	
    ...
    function winGame() {
		...
		document.getElementById( "score" ).innerHTML = "Score = " + scoreKK + " | Percent Correct = " + PercentTotal.toFixed(2) + "% | Remaining questions = " + NumOfQues; 
		document.getElementById("sent").style.textAlign = "center";
		document.getElementById( "sent" ).innerHTML = '<font style="color:#00FF00; font-size:2em;"></br>Correct</font>'; 
