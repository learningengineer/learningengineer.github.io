...
let FS2numQuestion = "0"; // question number
...
let FS2scoretotal = "0";  //The total score the user earns with correct answers.  
...
$(document).ready(() => {
	...
	$( "#start-over" ).click(function() {//this function runs when you click on one of the words.  
		window.localStorage.setItem("FS2qusNumber", "0"); 
		window.localStorage.setItem("FS2scrTotal", "0"); //Quit the quiz. The LMS will now store the data. 
		window.location.reload();

	});
	
	
	let nidx = window.localStorage.getItem("FS2qusNumber");
	...
	if (nidx > 0) { // If the variable nidx is greater than zero, meaning that the student didn't complete the quiz. 
		FS2scoretotal = window.localStorage.getItem("FS2scrTotal");
		FS2scoretotal = parseInt(FS2scoretotal);
		FS2numQuestion = nidxNum; //Push the number from the nidx variable into the FS2numQuestion variable. 
		FS2numQuestion = FS2numQuestion + 1;
	}		
	
	 window.onbeforeunload = function() { //When the user closes the window, do this.  

		//window.localStorage.setItem("qusNumber", nidx); 
		//window.localStorage.setItem("scrTotal", FS2scoretotal); //Quit the quiz. The LMS will now store the data.  
...
function newQuestion() {  //This is the function that runs when you click on the next-question button.  
			...
			splitwordarray = items[FS2numQuestion].sentence.split(" "); //This takes the sentence entry in the array and pushes each word separated by a space into an array.  
			splitanswerarray = items[FS2numQuestion].correct_answer.split(" "); //This takes the correct_answer entry in the array and pushes each number separated by a space into an array.  
			explanationtxt = items[FS2numQuestion].explanation; //The xplanationtext variable takes the explanation entry from the array. 
			linkwords = items[FS2numQuestion].link;  //The linkwords variable takes the explanation entry from the array. 
...
// The splitSentence function takes each element (word) in the array and creates a divider with a numbered id.  
			$('#score').html("Score: " + FS2scoretotal + " out of " + totalpossible); //Sets the score for the user. 
			$('#numQues').html("Question #" + (parseInt(FS2numQuestion) + 1) + " out of " + items.length);
			barWidth = Math.round(((FS2numQuestion + 1) / items.length) * 100); //creates the percentage for the completion bar.  
			...
			$( ".words" ).click(function() {//this function runs when you click on one of the words.  
				...
				if (splitanswerarray.includes(checkID)) {//This checks to see if the word's ID is in the splitanswerarray, and then it does what is below.  This checks to see if the user got the correct answer.   
					...
					if (correctCounter === splitanswerarray.length) {
						correctCounter = 0; 
						FS2scoretotal = parseInt(FS2scoretotal) + pointsperanswer; //This adds points to the users score.
						...
						$('#score').html("Score: " + FS2scoretotal + " out of " + totalpossible); //Changes the score div to reflect the new score. 
...
function savePlace() {
	//The code below is for SCORM 
			let numQues = parseInt(FS2numQuestion); //sets the numQues variable equal to the FS2numQuestion variable. 
			//This varible is probably unnecessary but I put it here to make these separate. 
			let rawscore = FS2scoretotal.toString(); //Turns the FS2scoretotal into a decimal
			let strQusNum = numQues.toString(); 
			window.localStorage.setItem("FS2qusNumber", strQusNum); 
			window.localStorage.setItem("FS2scrTotal", rawscore); //Quit the quiz. The LMS will now store the data. 
			FS2numQuestion = window.localStorage.getItem("FS2qusNumber");
			FS2numQuestion = parseInt(FS2numQuestion);
			if (FS2numQuestion >= 0) { //Checks to see that it was saved.  If true, the saveIndicator div flashes a green .
					...
					FS2numQuestion = FS2numQuestion + 1; //Adds 1 to the number of questions viewed.  
...
function gameOver() { //Runs when the game is 100% complete.  
	percentCorrect = Math.round((FS2scoretotal / totalpossible) * 100); //Gets the percent score using math round to a whole number.  
	let RawTotal = FS2scoretotal / totalpossible; //gets the RawTotal in a decimal point. 
	$('#sentences').html("GAME OVER!"); //Replaces the quiz questions with GAME OVER. 
	$('#link').html(""); //Empties this div
	$('#correct_incorrect').html("Score: " + FS2scoretotal + " out of " + totalpossible); //Reveals the final score.
	...
	localStorage.setItem("FS2qusNumber", numQues); 
	localStorage.setItem("FS2scrTotal", FS2scoretotal); //Quit the quiz. The LMS will now store the data. 
